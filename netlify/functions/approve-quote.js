...
function parseEdits(event, existingFields = {}) {
  ...
  const roundedSqFt = verifiedSqFt ? Math.round(verifiedSqFt) : null;
  const existingSqFt = Number(existingFields["Verified Sq Ft"]);
  const existingSource = clean(existingFields["Sq Ft Source"]);
  const manuallyConfirmed = roundedSqFt && (
    roundedSqFt !== existingSqFt
    || !existingSource
    || /no online building square footage/i.test(existingSource)
  );
  return {
    ...
    fields: {
      ...
      ...(roundedSqFt ? { "Verified Sq Ft": roundedSqFt } : {}),
      ...(manuallyConfirmed ? { "Sq Ft Source": "Anna confirmed during quote review" } : {})
    }
  };
}
...
exports.handler = async (event) => {
  ...
  try {
    ...
    return page(
      200,
      "Quote approved",
      `The approved quote for ${address} is now being sent to the client. This button can no longer be used again.`,
      recordUiUrl ? { href: recordUiUrl, label: "Open this job in Airtable" } : null
    );
  } catch (error) {
...
exports.airtableRecordUrl = airtableRecordUrl;
