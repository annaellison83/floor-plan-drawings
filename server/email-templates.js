...
const COMPANY_NAME = "FloorPlanDrawings";
const COMPANY_URL = "https://floorplandrawings.com";
...
function clientBrandHeaderHtml() {
  return `<table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="margin:0 0 26px;"><tr><td><a href="${COMPANY_URL}" style="display:inline-block;color:#173f36;text-decoration:none;"><span style="display:inline-block;vertical-align:middle;background:#173f36;color:#fff;border-radius:8px;padding:8px 9px;font-size:12px;line-height:14px;font-weight:700;letter-spacing:.08em;">FPD</span><span style="display:inline-block;vertical-align:middle;margin-left:10px;color:#173f36;font-size:18px;line-height:22px;font-weight:700;letter-spacing:.01em;">${COMPANY_NAME}</span></a><div style="margin:6px 0 0 47px;color:#6b7067;font-size:12px;line-height:18px;">Floor plans &amp; 3D tours</div></td></tr></table>`;
}

function clientBrandFooterText() {
  return `FloorPlanDrawings\n${COMPANY_URL}`;
}

function clientBrandFooterHtml() {
  return `<div style="margin-top:28px;padding-top:16px;border-top:1px solid #ddd7ca;color:#6b7067;font-size:12px;line-height:19px;"><a href="${COMPANY_URL}" style="color:#173f36;font-weight:700;text-decoration:none;">${COMPANY_NAME}</a> · <a href="${COMPANY_URL}" style="color:#173f36;text-decoration:underline;">${COMPANY_URL.replace("https://", "")}</a></div>`;
}
...
function safeInlineImageSource(value) {
  const candidate = text(value, "");
  return /^cid:[A-Za-z0-9._%+@-]+$/.test(candidate) ? candidate : "";
}
...
function imageCard(number, label, imageUrl, linkUrl, fallbackUrl) {
  ...
  const inline = safeInlineImageSource(imageUrl);
  const image = (candidate || inline) && !/earth\.google\.com|google\.com\/maps\/search/i.test(candidate || inline) ? (candidate || inline) : "";
...
function canonicalReviewEmail(job, options = {}) {
  ...
  const inlineAerial = safeInlineImageSource(job.emailAerialCid ? `cid:${job.emailAerialCid}` : "");
  const aerial = inlineAerial || safeUrl(stableAerial || job.emailAerialUrl || job.aerialAttachmentUrl || job.mapUrl);
  ...
  const image = (label, source, destination) => {
    const inlineSource = safeInlineImageSource(source);
    const valid = (safeUrl(source) || inlineSource) && !/earth\.google\.com|google\.com\/maps\/search/i.test(source);
    const target = safeUrl(destination) || safeUrl(source) || "";
    return `<td width="50%" valign="top" style="width:50%;padding:0 4px;"><a href="${escapeHtml(valid ? target : destination)}" target="_blank" style="display:block;color:#0b57d0;text-decoration:none;">${valid ? `<img src="${escapeHtml(source)}" alt="${escapeHtml(label)}" width="480" style="display:block;width:100%;max-width:100%;height:auto;border:0;border-radius:8px;">` : `<div style="padding:28px 8px;background:#e3eadf;border-radius:8px;text-align:center;font-size:13px;">Preview unavailable<br>Open ${escapeHtml(label.toLowerCase())}</div>`}</a></td>`;
  };
  ...
  const details = [
    ...
    row("Auto quote", pricing.finalPrice === null ? "Needs review" : money(pricing.finalPrice), true),
...
@media only screen and (max-width:640px){.canvas{width:100%!important;max-width:none!important}.pad{padding:12px!important}.card-pad{padding:22px 13px!important}.property-head{margin:16px 5px 0!important;padding:19px 16px!important}.address{font-size:23px!important;line-height:29px!important}.summary tr,.detail-cell{display:block!important;width:auto!important}.detail-cell{padding:5px!important}.detail{padding:16px!important}.size-lookup{margin:10px 5px 0!important;padding:16px!important}.quote{margin:13px 5px 0!important;padding:20px 14px!important}.quote-value{font-size:29px!important;line-height:35px!important}.image-wrap{padding:18px 5px 0!important}.availability{margin:18px 5px 0!important;padding:17px!important}.button{display:block!important;min-width:0!important;padding:15px 18px!important}.notes{margin:18px 5px 0!important;padding:17px!important}}
  ...
  const plainText = [
    ...
    `Auto quote: ${money(pricing.finalPrice)}`,
...
function clientQuoteEmail(job, proposalUrl = "", slots = []) {
  ...
  const plainText = [`Hi ${name},`, "We received your request for a floor plan.", `We charge by the square foot. Based on ${sizePhrase} at ${address}, pricing would be:`, quoteLines.length ? quoteLines.map(([label, amount]) => `${label}: ${amount}`).join("\n") : "Floor plan: To be confirmed", `Service: ${service}`, job.scope && `Scope: ${job.scope}`, options.length && options.map((slot, index) => `Option ${index + 1}: ${formatSlot(slot)}`).join("\n"), proposalUrl && `Choose an appointment time: ${proposalUrl}`, nextAvailable ? `Our next available appointment is ${nextAvailable}. Let me know if you would like to schedule.` : "If you would like to move forward, reply to this email with access details, and we will confirm it.", "Let me know if you would like to see examples or have any questions.", `Learn more about our services: ${COMPANY_URL}`, "Thank you,\nAnna", clientBrandFooterText()].filter(Boolean).join("\n\n");
